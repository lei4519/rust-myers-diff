...
fn main() {
	let old_str = r#"use std::collections::HashMap;

	/// myers 算法对比新旧文本 http://xmailserver.org/diff2.pdf
	pub fn myers(x_str: Vec<&str>, y_str: Vec<&str>) -> Vec<(Diff, String)> {
	...
	// diff 寻找路径
	fn diff(x_str: &Vec<&str>, y_str: &Vec<&str>) -> Vec<HashMap<isize, isize>> {
			// x 文本的长度
	...
	// 解析返回差异结果
	fn resolve_result(mut snakes: Snakes, x_str: Vec<&str>, y_str: Vec<&str>) -> Vec<(Diff, String)> {
			let mut result: Vec<(Diff, String)> = vec![];
			...
			let push = |result: &mut Vec<(Diff, String)>, action: Diff, v: &str| {
					...
					result.push((action, v.to_string()));
			};

			// 相同字符处理
			let advance = |result: &mut Vec<(Diff, String)>, mut x, mut y| {
	...
	mod test {
			...
			fn format_result(res: Vec<(Diff, String)>) -> String {
			...
			fn diff() {
					let res = myers(
							"ABCABBA".split("").filter(|x| !x.is_empty()).collect(),
							"CBABAC".split("").filter(|x| !x.is_empty()).collect(),
					);
	...
	"#;
	let new_str = r#"use std::collections::HashMap;
	...
	fn gen_snakes(d_kxmap: Vec<HashMap<isize, isize>>, x_max: isize, y_max: isize) -> Snakes {
			...
			while d > 0 {
					...
					let up_x = kx_map.get(&(k + 1)).map_or(-1, |x| *x);
					// 在上一个点向右走过来 (k - 1)
					let left_x = kx_map.get(&(k - 1)).map_or(-1, |y| *y);
	...
	let mut arr: Vec<u128> = Vec::with_capacity(10);
	let c = 10;
	let mut t: u128 = 0;
	for _ in 0..c  {
		let s = SystemTime::now();
		myers(old_str.split("").map(|x|x.to_string()).collect(), new_str.split("").map(|x|x.to_string()).collect());
		let e = s.elapsed().unwrap().as_millis();
		arr.push(e);
		t = t + e;
	}
	println!("{:?}", arr);
